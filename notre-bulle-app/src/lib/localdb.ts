...
/** Récupère les profils depuis Supabase et les met en cache local. */
export async function syncProfiles(profileIds: string[]): Promise<void> {
  ...
  try {
    ...
    for (const p of data) {
      await cacheProfile(p as any);
    }
  } catch {
    // Silencieux — on utilise le cache local existant
  }
}
