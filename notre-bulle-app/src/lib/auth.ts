...
/** Fenêtre de déverrouillage : après ce délai l'app se verrouille (comme le web). */
export const LOCK_WINDOW_MS = 24 * 60 * 60 * 1000;

// --- Fenêtre de déverrouillage 24 h (miroir du web) ---
export async function saveLastUnlock(): Promise<void> {
  await secureSet('notre-bulle.last-unlock', String(Date.now()));
}

export async function getLastUnlock(): Promise<number | null> {
  const val = await secureGet('notre-bulle.last-unlock');
  const ts = Number(val);
  return Number.isFinite(ts) ? ts : null;
}

// --- Epoch de session serveur (déconnexion forcée des autres appareils) ---
export async function saveSessionEpoch(epoch: number): Promise<void> {
  await secureSet('notre-bulle.session-epoch', String(epoch));
}

export async function getSessionEpoch(): Promise<number | null> {
  const val = await secureGet('notre-bulle.session-epoch');
  const n = Number(val);
  return Number.isFinite(n) ? n : null;
}

// --- Hachage PIN (SHA-256) — legacy, conservé pour compat ---
...
// --- PIN temporaire pour re-link RLS à chaque démarrage ---
const TEMP_PIN_KEY = 'notre-bulle.temp-pin';

/** Stocke le PIN temporairement (utile pour re-lier auth_user_id au démarrage). */
export async function saveTempPin(pin: string): Promise<void> {
  await secureSet(TEMP_PIN_KEY, pin);
}

/** Récupère le PIN temporaire (retourne null si absent ou expiré). */
export async function getTempPin(): Promise<string | null> {
  return secureGet(TEMP_PIN_KEY);
}

/** Efface le PIN temporaire (déconnexion). */
export async function clearTempPin(): Promise<void> {
  await secureSet(TEMP_PIN_KEY, '');
}
