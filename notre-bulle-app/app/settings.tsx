...
} from '../src/lib/ringtones';
import {
  BackIcon, SettingsIcon, HeartFilledIcon, UserIcon, LockIcon,
  EditIcon, CameraIcon, CheckIcon, CloseIcon, ImageIcon, PhoneRingIcon, PlayIcon,
...
// ==========================================
// RINGTONE PICKER — son du téléphone / intégrées / musique perso
// ==========================================
function RingtonePicker({ onToast }: { onToast: (msg: string) => void }) {
  const [selected, setSelected] = useState<string>(DEFAULT_RINGTONE);
  const [busy, setBusy] = useState(false);
  const stopPreviewRef = useRef<(() => Promise<void>) | null>(null);
  const [customLabel, setCustomLabel] = useState<string | null>(null);

  // Charger le choix enregistré + le nom de la musique perso
  useEffect(() => {
    (async () => {
      try {
        const stored = await getStoredRingtone();
        setSelected(stored);
        if (isCustomMusic(stored)) {
          const name = await getSetting('ringtone.name');
          setCustomLabel(name || 'Ma musique');
        }
      } catch {}
    })();
    return () => { void stopPreviewRef.current?.(); };
  }, []);
  ...
  const handleSelect = async (id: string) => {
    ...
    try {
      ...
      const info = await applyRingtone(id);
      setSelected(id);
      setCustomLabel(null);
      onToast(info ?? 'Sonnerie appliquée');
    } catch (err) {
      console.warn('[RingtonePicker]', err);
    } finally {
      setBusy(false);
    }
  };

  // Musique perso via document picker
  const handlePickMusic = async () => {
    ...
    try {
      ...
      try {
        await stopPreviewRef.current?.();
        stopPreviewRef.current = null;
        const info = await applyRingtone(asset.uri);
        const { setSetting } = await import('../src/lib/settings');
        await setSetting('ringtone.name', asset.name || 'Ma musique');
        setSelected(asset.uri);
        setCustomLabel(asset.name || 'Ma musique');
        onToast(info ?? 'Musique appliquée');
      } finally {
        setBusy(false);
      }
    } catch (err) {
      console.warn('[RingtonePicker] pick:', err);
      setBusy(false);
    }
  };

  // Aperçu d'une sonnerie intégrée
  const handlePreview = async (id: string) => {
    if (busy) return;
    try {
      await stopPreviewRef.current?.();
      stopPreviewRef.current = await playRingtoneLoop(id, { force: true });
    } catch {}
  };
  ...
  return (
    <View>
      {RINGTONES.map((rt) => (
        <TouchableOpacity
          key={rt.id}
          onPress={() => handleSelect(rt.id)}
          style={styles.ringRow}
          activeOpacity={0.7}
          disabled={busy}
        >
          <View style={[styles.ringRadio, isSelected(rt.id) && styles.ringRadioSelected]} />
          <Text style={[styles.ringLabel, isSelected(rt.id) && styles.ringLabelSelected]}>
            {rt.label}
          </Text>
          {rt.asset && (
            <TouchableOpacity
              onPress={() => handlePreview(rt.id)}
              style={styles.ringPreviewBtn}
      ...
      <TouchableOpacity
        ...
        disabled={busy}
      >
        <View style={[styles.ringRadio, isCustomMusic(selected) && styles.ringRadioSelected]} />
        <Text style={[styles.ringLabel, isCustomMusic(selected) && styles.ringLabelSelected]} numberOfLines={1}>
          {isCustomMusic(selected) ? customLabel || 'Ma musique' : 'Choisir ma musique…'}
        </Text>
      </TouchableOpacity>

      <Text style={styles.ringHint}>
        Les sonneries intégrées sonnent même quand Notre Bulle est fermée.
        Ta musique perso sonne quand l&apos;app est ouverte ou en arrière-plan.
      </Text>
    </View>
  );
}
...
export default function SettingsScreen() {
  ...
  const { changePinOnServer, logout } = useAuth();
  ...
  // Get own profile ID (pour photo, fond d'ecran)
  const getUserId = useCallback(async (): Promise<string | null> => {
    return getOwnProfileId();
  }, []);

  // Get PARTNER profile ID (on modifie le pseudo du partenaire)
  const getPartnerId = useCallback(async (): Promise<string | null> => {
    return getActualPartnerProfileId();
  }, []);
  ...
  // Save PARTNER display name (via RPC sécurisé)
  const saveDisplayName = useCallback(async () => {
    ...
    try {
      const { data: rpcData, error: rpcErr } = await supabase.rpc('update_partner_display_name', {
        p_new_name: newDisplayName.trim(),
      });
      if (rpcErr) throw rpcErr;
      const row = Array.isArray(rpcData) ? rpcData[0] : rpcData;
      if (row?.ok === false) throw new Error(row?.error || 'Erreur serveur');
  ...
  }, [newDisplayName, showToast, showAlert, savingPseudo]);

  // PIN handler — vérification et changement CÔTÉ SERVEUR (RPC)
  // même base de données que le web : profiles.pin_hash via change_couple_pin
  const handlePinKey = useCallback(async (key: string) => {
    ...
    if (pinStep === 'old' && oldPin.length < 4) {
      ...
      setOldPin(newVal);
      if (newVal.length === 4) {
        // Vérification serveur du code actuel (sans révéler le hash)
        try {
          const { getOwnProfileId } = await import('../src/lib/profile');
          const profileId = await getOwnProfileId();
          if (!profileId) throw new Error('Profil introuvable');
          const res = await supabase.rpc('verify_couple_pin', { p_profile_id: profileId, p_pin: newVal });
          ...
          setPinError(err?.message || 'Erreur serveur');
          setTimeout(() => setOldPin(''), 400);
        }
      }
      ...
      if (newVal.length === 4) {
        if (newVal === newPin) {
          const result = await changePinOnServer(oldPin, newVal);
          if (result.ok) {
            ...
            setPinError(result.error || 'Erreur serveur');
            setTimeout(() => { setOldPin(''); setNewPin(''); setConfirmPin(''); setPinStep('old'); }, 800);
          }
        } else {
  ...
  }, [pinStep, oldPin, newPin, confirmPin, showToast, changePinOnServer]);
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        {/* --- Sonnerie d'appel --- */}
        <SectionCard title="Sonnerie d appel" icon={PhoneRingIcon} delay={300}>
          <RingtonePicker onToast={showToast} />
        </SectionCard>
        ...
        >
          <Text style={{ color: '#991B1B', fontSize: 16, fontWeight: '600' }}>
            Déconnexion
          </Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  ringHint: {
    ...typography.caption, color: colors.textTertiary,
    marginTop: spacing.sm, fontStyle: 'italic', lineHeight: 17,
  },
