...
export default function SetupPinScreen() {
  const { checkAuth, createPinOnServer } = useAuth();
  ...
  const handleKeyPress = useCallback(
    async (key: string) => {
      ...
      if (step === 'confirm') {
        ...
        if (newConfirm.length === PIN_LENGTH) {
          if (newConfirm === pin && !isSavingPin) {
            // 🔐 Le PIN est créé CÔTÉ SERVEUR (RPC set_couple_pin) —
            // même base de données que le web.
            setIsSavingPin(true);
            createPinOnServer(newConfirm).then((result) => {
              ...
              if (result.ok) {
                ...
                // Un code secret existe déjà côté serveur (créé sur un autre
                // appareil) → retour à l'écran de déverrouillage pour l'entrer.
    ...
    [step, pin, confirmPin, createPinOnServer, isSavingPin]
  );
